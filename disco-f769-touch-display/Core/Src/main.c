...
int main(void)
{
  ...
  BSP_LCD_DisplayStringAt(0, LINE(1), (uint8_t *)"STM32F769i Discovery Kit", CENTER_MODE);
  BSP_LCD_DisplayStringAt(0, LINE(2), (uint8_t *)"Touch-Display Demo", CENTER_MODE);
  ...
  BSP_LCD_DisplayStringAt(30, LINE(5), (uint8_t*)"Touch display coords:", LEFT_MODE);
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (TS_State.touchDetected) {
	    ...
	    uint16_t xAxis = TS_State.touchX[0];
	  	...
	  	// print coords via uart
	  	printf("%d, %d\r\n", xAxis, yAxis);

	  	// display coords on screen
	  	sprintf(buffer, "Touch display coords: x=%3d, y=%3d", xAxis, yAxis);
	  	BSP_LCD_DisplayStringAt(30, LINE(5) , (uint8_t*)&buffer, LEFT_MODE);
	  }
  }
  /* USER CODE END 3 */
}
